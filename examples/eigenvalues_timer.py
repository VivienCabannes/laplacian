...
if graph_laplacian:
    ...
    savetime_file = SAVE_DIR / f"eigenvalues_{name}_graph_time.npy"
else:
    save_file = SAVE_DIR / f"eigenvalues_{name}.npy"
    savetime_file = SAVE_DIR / f"eigenvalues_{name}_time.npy"

np.random.seed(100)
ns = np.logspace(1.5, 3, num=10).astype(int)
N = np.max(ns)
ps = np.linspace(.1, 1, num=10)
...
for num_trial in range(nb_trials):
    ...
    write_numpy_file(times, savetime_file)
